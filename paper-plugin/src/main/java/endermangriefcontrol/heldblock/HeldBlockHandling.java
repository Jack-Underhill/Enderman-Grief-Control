 ...
 * handled once found, per world. These are mutually exclusive - alerting ("go hunt it yourself")
 * and auto-clearing ("resolved for you") are contradictory intents for the same enderman, not
 * independent settings to stack.
 */
public enum HeldBlockHandling {
    /** Leave it alone - don't log, don't clear. */
    OFF,
    /** Periodically re-log its location so it can be manually hunted down and killed. */
    ALERT,
